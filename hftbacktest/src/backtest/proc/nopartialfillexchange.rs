    ...
    fn check_if_sell_filled(
        ...
        match order.price_tick.cmp(&price_tick) {
            ...
            Ordering::Less => {
                ...
                // NoPartialFill semantics: any remaining trade qty (> lot_size) triggers a full
                // fill. Deduct the full order qty from the pool to prevent the same trade from
                // filling multiple orders beyond its total qty.
                if *remaining_qty <= self.depth.lot_size() {
                    return Ok(());
                }
